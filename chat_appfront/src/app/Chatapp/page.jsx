...
function Chatapp() {
  ...
  const [totalmessage2, setTotalmessage2] = useState();
  ...
  useEffect(() => {
    ...
    axios
      .post("http://127.0.0.1:8000/inbox", { from_user: id })
      .then((res) => {
        console.log(res.data);
        setSenduser(res.data.data.send_usernames);
        setreceiveUser(res.data.data.receive_usernames);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  function removeDuplicates(arr) {
    ...
    // Filter the array, keeping only the first occurrence of each username
    const result = arr.filter((item) => {
      const username = item.username;

      if (!uniqueUsernames.has(username)) {
        uniqueUsernames.set(username, true);
        return true;
      }
  ...
  useEffect(() => {
    ...
    if (senduser) {
      const resultArray = removeDuplicates(
        Array.from(new Set(senduser.concat(receiveuser)))
      );

      setTotaluser(resultArray);
    }
  }, [senduser, receiveuser]);
  const setmessage = (user_id) => {
    axios
      .post("http://127.0.0.1:8000/user_chat", {
        from_user: id,
        to_user: user_id,
      })
      .then((res) => {
        console.log(res);
        setSendmessage(res.data.send);
        setReceivemessage(res.data.receive);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    if (sendmessage) {
      setTotalmessage(sendmessage.concat(receivemessage));
      console.log(totalmessage, "gggggg");
      
    }
  }, [sendmessage, receivemessage]);
  useEffect(()=>{
    if(totalmessage){
        const sortedArray = totalmessage.sort((a, b) => new Date(a.date) - new Date(b.date));
        setTotalmessage2(sortedArray)

console.log(sortedArray,'ddddddd');
    }
  },[totalmessage])
            ...
            <div class="flex border border-grey rounded shadow-lg h-full">
              <div class="w-1/3 border flex flex-col">
                ...
                <div class="bg-grey-lighter flex-1 overflow-auto">
                  {totaluser?.map((value, key) => (
                    <div
                      onClick={() => setmessage(value.id)}
                      key={key}
                    ...
                    >
                      ...
                      <div class="ml-4 flex-1 border-b border-grey-lighter py-4">
                        <div class="flex items-bottom justify-between">
                          <p class="text-grey-darkest">{value.username}</p>
